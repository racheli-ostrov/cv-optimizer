...
export default function CVOptimizer() {
  ...
  const downloadImprovedPDF = async (content) => {
    ...
    try {
      const response = await fetch("http://localhost:3000/api/download-improved-pdf", {
        ...
        body: JSON.stringify({ improvedContent: content }),
      });
    ...
    if (awaitingImproveAnswer) {
      if (answer === "כן" || answer === "yes") {
        if (cvSuggestions.length > 0) {
          let formattedSuggestions = cvSuggestions
            .map((s, i) => `${i + 1}. ${s}`)
            .join("\n");
          addMessage(`הקורות חיים שלך מצוינים! הנה ההמלצות:\n\n${formattedSuggestions}`, "ai");
        } else {
          addMessage("אין לי הערות לשיפור, קורות החיים שלך מצוינים!", "ai");
        }
        setAwaitingImproveAnswer(false);
        setAwaitingDownloadAnswer(true);
        addMessage("שנוציא יחד קובץ חדש ומשוכלל יותר של קורות חיים בשבילך?", "ai");
        addMessage("שים לב!, אם  הקובץ בפונט שאינו ברירת מחדל הקובץ המשודרג ירד בתבנית בלבד!!", "ai");
      } else if (answer === "לא" || answer === "no") {
        addMessage("חבל מאוד— אפשר תמיד להעלות שוב.", "ai");
        setAwaitingImproveAnswer(false);
      } else {
        addMessage('🤔 לא הבנתי אם כן או לא', "ai");
      }
      setInput("");
      return;
    }
      ...
      setTimeout(() => {
        addMessage(
          waitingCount === 0
            ...
            : "קורות חיים חביבי, לא סיפורי חיים 😉",
          "ai"
        );
      }, 400);
      setInput("");
  ...
  const handleFileUpload = (file) => {
    ...
    setTimeout(() => sendFileToServer(file), 500);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto fade-in" style={{ paddingBottom: "6rem" }}>
      <h3 className="text-3xl font-bold mb-4" style={{ color: "var(--gemini-indigo)" }}>
        🐾 ChatCV — הבה נשדרג את הקורות חיים שלך
      </h3>
